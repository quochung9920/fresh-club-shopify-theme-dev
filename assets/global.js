...
class QuantityInputCustom extends HTMLElement {
  constructor() {
    ...
    this.querySelector('.submit_btn.btn')?.addEventListener('click', this.addItemToCart.bind(this));
  }
  ...
  connectedCallback() {
    this.validateQtyRules();
    this.quantityUpdateUnsubscriber = subscribe(PUB_SUB_EVENTS.quantityUpdate, this.resetInputValues.bind(this));
  }
  ...
  addItemToCart(event) {
    event.preventDefault();
    //go to function validateQuantity() of class BulkAdd below
...
class BulkAdd extends HTMLElement {
  ...
  sendRequest(queue) {
    ...
    // instead of updating products in bulk, use add products in bulk instead. Go to addMultipleQty() in quick-add-bulk.js
    const items = [];
    queue.forEach((queueItem) => {
      items.push({
        id: queueItem.id,
